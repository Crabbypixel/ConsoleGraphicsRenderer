...
/*
	Thanks to the OLC Community for this library. I have modified this according to my own needs.
*/
...
#ifdef CF_GRAPHICS
#error "cfGeometryLib.h must be included before ConsoleGraphics.h"
#endif
...
namespace cf::geom2d
{
	...
	{
		template<typename T>
		inline std::vector<cf::vec_2d<T>> filter_duplicate_points(const std::vector<cf::vec_2d<T>>& points) {
			std::vector<cf::vec_2d<T>> filtered_points;
	...
	struct line
	{
		...
		inline line(const cf::vec_2d<T>& s = { T(0), T(0) },
			const cf::vec_2d<T>& e = { T(0), T(0) })
			: start(s), end(e)
		{ }

		// Get vector pointing from start to end
		inline constexpr cf::vec_2d<T> vector() const
		{
			return (end - start);
		}
		...
		inline constexpr cf::vec_2d<T> rpoint(const T& distance) const
		{
			return start + vector().norm() * distance;
		}

		// Given a unit distance, get point along line
		inline constexpr cf::vec_2d<T> upoint(const T& distance) const
		{
			return start + vector() * distance;
		}

		// Return which side of the line does a point lie
		inline constexpr int32_t side(const cf::vec_2d<T>& point) const
		{
		...
		inline constexpr cf::vd2d coefficients() const
		{
			double x1 = start.x;
			double x2 = end.x;
			double y1 = start.y;
			...
			if (std::abs(x2 - x1) < epsilon) {
				return cf::vd2d{ std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity() };
			}

			double m = (y2 - y1) / (x2 - x1);
			return cf::vd2d{ m, -m * x1 + y1 };
		}
	};
	...
	{
		cf::vec_2d<T> origin;
		cf::vec_2d<T> direction;

		inline ray(const cf::vec_2d<T>& o = { T(0), T(0) },
			const cf::vec_2d<T>& d = { T(0), T(0) })
	...
	struct rect
	{
		...
		inline rect(const cf::vec_2d<T>& p = { T(0), T(0) },
			const cf::vec_2d<T>& s = { T(1), T(1) })
			: pos(p), size(s)
		{ }

		inline cf::vec_2d<T> middle() const
		{
	...
	struct circle
	{
		...
		inline circle(const cf::vec_2d<T>& p = { T(0), T(0) }, const T r = T(0))
			: pos(p), radius(r)
		{ }
	...
	struct triangle
	{
		std::array<cf::vec_2d<T>, 3> pos;

		inline triangle(
			const cf::vec_2d<T>& p0 = { T(0), T(0) },
			const cf::vec_2d<T>& p1 = { T(0), T(0) },
			const cf::vec_2d<T>& p2 = { T(0), T(0) })
			: pos{ p0,p1,p2 }
		{ }
	...
	inline cf::vec_2d<T1> closest(const cf::vec_2d<T1>& p, [[maybe_unused]] const T2& anything_who_cares)
	{
		return p;
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const line<T1>& l, const cf::vec_2d<T2>& p)
	{
		auto d = l.vector();
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const circle<T1>& c, const cf::vec_2d<T2>& p)
	{
		return c.pos + cf::vd2d(p - c.pos).norm() * c.radius;
	}

	// closest(r,p)
	// Returns closest point on rectangle to point
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const rect<T1>& r, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const triangle<T1>& t, const cf::vec_2d<T2>& p)
	{
		cf::geom2d::line<T1> l{ t.pos[0], t.pos[1] };
		auto p0 = closest(l, p);
		auto d0 = (p0 - p).mag2();
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const ray<T1>& r, const cf::vec_2d<T2>& p)
	{
		// TODO: implement
		return p;
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const line<T1>& l1, const line<T2>& l2)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const rect<T1>& r, const line<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const circle<T1>& c, const line<T2>& l)
	{
		const auto p1 = closest(l, c.pos);
		return c.pos + cf::vd2d(p1 - c.pos).norm() * c.radius;
	}

	// closest(t,l)
	// Returns closest point on triangle to line
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const triangle<T1>& t, const line<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const line<T1>& l, const circle<T2>& c)
	{
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const rect<T1>& r, const circle<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const circle<T1>& c1, const circle<T2>& c2)
	{
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const triangle<T1>& r, const circle<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const line<T1>& l, const triangle<T2>& c)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const rect<T1>& r, const triangle<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const circle<T1>& c, const triangle<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline cf::vec_2d<T1> closest(const triangle<T1>& r, const triangle<T2>& l)
	{
		// TODO:
		return {};
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool contains(const cf::vec_2d<T1>& p1, const cf::vec_2d<T2>& p2)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool contains(const line<T1>& l, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool contains(const rect<T1>& r, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool contains(const circle<T1>& c, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool contains(const triangle<T1>& t, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool contains(const ray<T1>& r, const cf::vec_2d<T2>& p)
	{
		// Calculate the vector from the ray's origin to point p
		cf::vec_2d<T2> op = p - r.origin;
		...
		cf::vec_2d<T2> projection = { r.direction.x * dotProduct, r.direction.y * dotProduct };
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const cf::vec_2d<T1>& p1, const cf::vec_2d<T2>& p2)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const line<T1>& l, const cf::vec_2d<T2>& p)
	{
		return contains(l, p);
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const rect<T1>& r, const cf::vec_2d<T2>& p)
	{
		return contains(r, p);
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const circle<T1>& c, const cf::vec_2d<T2>& p)
	{
		return contains(c, p);
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const triangle<T1>& t, const cf::vec_2d<T2>& p)
	{
		return contains(t, p);
	}
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const cf::vec_2d<T1>& p1, const cf::vec_2d<T2>& p2)
	{
		if (contains(p1, p2))
			return { p1 };
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const line<T1>& l, const cf::vec_2d<T2>& p)
	{
		if (contains(l, p))
			return { p };
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const rect<T1>& r, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const circle<T1>& c, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const triangle<T1>& t, const cf::vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool contains([[maybe_unused]] const cf::vec_2d<T1>& p, [[maybe_unused]] const line<T2>& l)
	{
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const cf::vec_2d<T1>& p, const line<T2>& l)
	{
		return contains(l, p);
	}
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const cf::vec_2d<T1>& p, const line<T2>& l)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const line<T1>& l1, const line<T2>& l2)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const rect<T1>& r, const line<T2>& l)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const circle<T1>& c, const line<T2>& l)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const triangle<T1>& t, const line<T2>& l)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline constexpr bool contains([[maybe_unused]] const cf::vec_2d<T1>& p, [[maybe_unused]] const rect<T2>& r)
	{
		return false; // It can't!
	}
	...
	inline constexpr bool contains(const circle<T1>& c, const rect<T2>& r)
	{
		return contains(c, r.pos)
			&& contains(c, cf::vec_2d<T2>{ r.pos.x + r.size.x, r.pos.y })
			&& contains(c, cf::vec_2d<T2>{ r.pos.x, r.pos.y + r.size.y })
	...
	inline constexpr bool contains(const triangle<T1>& t, const rect<T2>& r)
	{
		return contains(t, r.pos)
			...
			&& contains(t, cf::vec_2d<T2>{ r.pos.x + r.size.x, r.pos.y })
			&& contains(t, cf::vec_2d<T2>{ r.pos.x, r.pos.y + r.size.y });
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const cf::vec_2d<T1>& p, const rect<T2>& r)
	{
		return overlaps(r, p);
	}
	...
	inline constexpr bool overlaps(const circle<T1>& c, const rect<T2>& r)
	{
		...
		double overlap = (cf::vec_2d<T2>{ std::clamp(c.pos.x, r.pos.x, r.pos.x + r.size.x), std::clamp(c.pos.y, r.pos.y, r.pos.y + r.size.y) } - c.pos).mag2();
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const cf::vec_2d<T1>& p, const rect<T2>& r)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const line<T1>& l, const rect<T2>& r)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const rect<T1>& r1, const rect<T2>& r2)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const circle<T1>& c, const rect<T2>& r)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const triangle<T1>& t, const rect<T2>& r)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline constexpr bool contains([[maybe_unused]] const cf::vec_2d<T1>& p, [[maybe_unused]] const circle<T2>& c)
	{
		return false; // It can't!
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const cf::vec_2d<T1>& p, const circle<T2>& c)
	{
		return overlaps(c, p);
	}
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const cf::vec_2d<T1>& p, const circle<T2>& c)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const line<T1>& l, const circle<T2>& c)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const rect<T1>& r, const circle<T2>& c)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const circle<T1>& c1, const circle<T2>& c2)
	{
		...
		vec_2d<T1> between = c2.pos - c1.pos;
	  ...
	  //                                                                                                                   
		...
		vec_2d<T1> chordCenter = c1.pos + between.norm() * ccDist;
		vec_2d<T1> halfChord = between.norm().perp() * std::sqrt(c1.radius * c1.radius - ccDist * ccDist);
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const triangle<T1>& t, const circle<T2>& c)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline constexpr bool contains([[maybe_unused]] const cf::vec_2d<T1>& p, [[maybe_unused]] const triangle<T2>& t)
	{
		return false; // It can't!
	}
	...
	template<typename T1, typename T2>
	inline constexpr bool overlaps(const cf::vec_2d<T1>& p, const triangle<T2>& t)
	{
		return overlaps(t, p);
	}
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const cf::vec_2d<T1>& p, const triangle<T2>& t)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const line<T1>& l, const triangle<T2>& t)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const rect<T1>& r, const triangle<T2>& t)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const circle<T1>& c, const triangle<T2>& t)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const triangle<T1>& t1, const triangle<T2>& t2)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	inline constexpr circle<T1> envelope_c(const cf::vec_2d<T1>& p)
	{
	...
	template<typename T1>
	inline constexpr rect<T1> envelope_r(const cf::vec_2d<T1>& p)
	{
	...
	{
		return rect<T1>(c.pos - vec_2d<T1>{c.radius, c.radius}, { c.radius * 2, c.radius * 2 });
	}
	...
	inline std::optional<cf::vec_2d<T1>> project(const circle<T1>& c, const cf::vec_2d<T2>& p, const ray<T3>& q)
	{
	...
	template<typename T1, typename T2, typename T3>
	inline std::optional<cf::vec_2d<T1>> project(const circle<T1>& c1, const circle<T2>& c2, const ray<T3>& q)
	{
	...
	template<typename T1, typename T2, typename T3>
	inline std::optional<cf::vec_2d<T2>> project(const circle<T1>& c, const line<T2>& l, const ray<T3>& q)
	{
		...
		std::vector<cf::vec_2d<T2>> vAllIntersections;
	...
	template<typename T1, typename T2, typename T3>
	inline std::optional<cf::vec_2d<T2>> project(const circle<T1>& c, const rect<T2>& r, const ray<T3>& q)
	{
	...
	template<typename T1, typename T2, typename T3>
	inline std::optional<cf::vec_2d<T2>> project(const circle<T1>& c, const triangle<T2>& t, const ray<T3>& q)
	{
		// TODO:
		return std::nullopt;
	}
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const ray<T1>& q1, const ray<T2>& q2)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const ray<T1>& q, const vec_2d<T2>& p)
	{
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const ray<T1>& q, const line<T2>& l)
	{
	...
	template<typename T1, typename T2>
	inline std::optional<std::pair<cf::vec_2d<T2>, cf::vec_2d<T2>>> collision(const ray<T1>& q, const line<T2>& l)
	{
	...
	template<typename T1, typename T2>
	inline std::optional<ray<T1>> reflect(const ray<T1>& q, const cf::vec_2d<T2>& p)
	{
		// TODO:
		return std::nullopt;
	}
	...
	// optionally returns collision point and collision normal of ray and a line segment, if it collides
	template<typename T1, typename T2>
	inline std::optional<std::pair<cf::vec_2d<T1>, cf::vec_2d<T1>>> collision(const ray<T1>& q, const rect<T2>& r)
	{
		cf::vec_2d<T1> vClosestIntersection;
		cf::vec_2d<T1> vIntersectionNormal;
	...
	template<typename T1, typename T2>
	inline std::optional<std::pair<cf::vec_2d<T2>, cf::vec_2d<T2>>> collision(const ray<T1>& q, const circle<T2>& c)
	{
	...
	template<typename T1, typename T2>
	inline std::optional<std::pair<cf::vec_2d<T1>, cf::vec_2d<T1>>> collision(const ray<T1>& q, const triangle<T2>& t)
	{
		cf::vec_2d<T1> vClosestIntersection;
		cf::vec_2d<T1> vIntersectionNormal;
		double dClosestDistance2 = std::numeric_limits<double>::max();
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const ray<T1>& q, const circle<T2>& c)
	{
		// Look familiar?
		double A = q.direction.mag2();
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const ray<T1>& q, const rect<T2>& r)
	{
		std::vector<cf::vec_2d<T2>> intersections;
	...
	template<typename T1, typename T2>
	inline std::vector<cf::vec_2d<T2>> intersects(const ray<T1>& q, const triangle<T2>& t)
	{
		std::vector<cf::vec_2d<T2>> intersections;
